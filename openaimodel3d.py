...
class ResBlock(TimestepBlock):
    ...
    def _forward(self, x, emb,):
        ...
        output_shape = out.shape    # 추가

        print(f'ResBlock - input shape: {input_shape} / output shape: {output_shape}')
...
class UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps=None, time_emb_replace=None, context=None, y=None, **kwargs):
        ...
        print(f'input shape: {x.shape}') # input dimension 확인 
        ...
        for i, module in enumerate(self.input_blocks):
            ...
            print(f'input block 통과 이후 dimension [{i+1}]" {h.shape}') # input block 통과 후
            hs.append(h)

        h = self.middle_block(h, emb, context, **kwargs)
        print(f'middle block 통과 이후 dimension" {h.shape}') # 

        for module in self.output_blocks:
            h = th.cat([h, hs.pop()], dim=1) # skip connection을 위한 concat 
            print(f'skip connection 이후 dimension [{i+1}]: {h.shape}')
            h = module(h, emb, context, **kwargs)
            print(f'output block 통과 이후 dimension [{i+1}]" {h.shape}') # output block 통과 후 

        h = h.type(x.dtype)
        output = self.out(h)
        print(f'최종 output: {output.shape}')
