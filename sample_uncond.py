...
def get_parser():
    ...
    parser.add_argument("--show_denoising_progress", action='store_true', default=True, help="whether show denoising progress during sampling one batch",)
...
def main():
    ...
    opt_args = vars(opt).copy()
    opt_args.pop('batch_size', None)

    samples = sample(model, noise_shape, n_iters, sampler=ddim_sampler, **opt_args) # batch_size patameter를 두번 전달하면 안됨
